...
public partial class SearchPageViewModel : ViewModelBase
{
    ...
    /// </summary>
    private void ApplySecondaryFilter(int skip = 0)
    {
        // 没有选择任何二次分类时，直接恢复显示全部原始搜索结果
        if (Secondary.Count == 0)
        {
           FileSystemItems.AddRange(new ObservableCollection<object>(FileSystemItemsInit.Skip(skip).Take(50)));
            return;
        }
        ...
        var filtered = FileSystemItemsInit.Skip(skip).Take(50).Where(item => item switch
        {
        ...
        // 用筛选后的结果替换当前界面显示集合
       FileSystemItems.AddRange( new ObservableCollection<object>(filtered));
    }
                ...
                if (message.parameter is CustomView v)
                {
                    if (v.Type == 0)
                    {
                        ...
                        if (info.Status == 0)
                        {
                            ...
                            FileSystemItems.Clear();
                            SkipNum = 0;
                            Number = FileSystemItemsInit.Count;
                            ApplySecondaryFilter();
                        }
                        else
                        {
    ...
    private async Task SearchFile()
    {
        try
        {
            ...
            {
                //空值拦截
                if (string.IsNullOrEmpty(SearchInfo.Keyword) && SearchInfo.FileType.Count == 0 &&
                    SearchInfo.EndCreationTime == null &&
                    SearchInfo.FileSizeInBytesMax == null && SearchInfo.FileSizeInBytesMin == null &&
                    SearchInfo.StarCreationTime == null &&
